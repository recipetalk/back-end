...
public class SendMailServiceImpl implements SendMailService {
    ...
    private MimeMessage createEmailForm(String toEmail) throws MessagingException {
        //인증 토큰 생성
        VerificationToken verificationToken = verificationTokenService.createVerificationToken(toEmail);
        MimeMessage message = emailSender.createMimeMessage();
        message.addRecipients(Message.RecipientType.TO, toEmail);
        message.setSubject(title);
        message.setFrom(username+"@naver.com");
        message.setText(setContext(verificationToken.getToken()), "utf-8", "html");
    ...
    private String setContext(String verificationToken){
        ...
        return templateEngine.process("templates/mail", context);
    }
